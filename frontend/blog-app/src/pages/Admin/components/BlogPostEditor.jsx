...
const BlogPostEditor = ({isEdit}) => {
  ...
  const generatePostIdeas=async ()=>{
      setIdeaLoading(true)
      try {
        const aiResponse=await axiosInstance.post(API_PATHS.AI.GENERATE_BLOG_IDEAS,
          {
    ...
    if(!isDraft){
      ...
      try {
        ...
        await axiosInstance.put(
          ...
          rePayload
        ):await axiosInstance.post(API_PATHS.POSTS.CREATE,rePayload)
  ...
  const fetchPostDetailsBySlug=async ()=>{
    try {
      ...
      console.log("Param value:", postSlug);
          ...
          <div className='form-card p-6 col-span-12 md:col-span-8'>
            <div className='flex items-center justify-between'>
              ...
              <div className='flex items-center gap-3'>
                {isEdit && (
                  <button
                  className='flex items-center gap-2.5 text-[13px] font-medium text-rose-500 bg-rose-50/60 rounded px-1.5 md:px-3 py-1 md:py-[3px] border border-rose-50 hover:border-rose-300 cursor-pointer scale-[1.02] transition-all'
                  disabled={loading}
